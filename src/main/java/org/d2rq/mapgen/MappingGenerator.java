	...
	public void generate(Target generationTarget) {
		...
		for (TableName tableName: tableNames) {
			...
			if (handleLinkTables && isLinkTable(table)) {
				...
				// Determine direction of the generated bridge:
				// Look at first column; the table referenced by it will become
				// the subject map. If for whatever reason it references both or
				// neither table, try the next column. If we run out of columns,
				// the two foreign keys are identical, and their order doesn't
				// matter.
				IdentifierList columns1 = fk1.getLocalColumns();
				IdentifierList columns2 = fk2.getLocalColumns();
				for (Identifier column: table.getColumnNames()) {
					if (columns1.contains(column) == columns2.contains(column)) continue;
